...
env = ToyEnv(
    ...
    down_reward=y,
    up_reward=y,
    # up_reward=1, # TODO: For debugging, should be easy to learn
    n_states=n_states,
    big_reward=10.0,
...
actor = QValueActor(qvalue_module, in_keys=["state"], spec=env.action_spec)
...
wandb.init(project="clean-base-dqn")
...
try:
    for i, tensordict_data in enumerate(collector):
        ...
        if i % eval_every_n_batch == 0:
            with torch.no_grad(), set_exploration_type(InteractionType.DETERMINISTIC):
                eval_data = env.rollout(100, actor)
